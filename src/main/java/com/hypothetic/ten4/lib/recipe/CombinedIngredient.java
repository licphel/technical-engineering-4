...
public class CombinedIngredient {
  public static final Codec<CombinedIngredient> CODEC = new Codec<>() {
    @Override
    ...
    @Override
    public <T> DataResult<T> encode(CombinedIngredient input, DynamicOps<T> ops, T prefix) {
  ...
  private CombinedIngredient(Kind kind, ResourceLocation id, int count, double chance) {
    this.kind = kind;
    this.id = id;
    this.count = count;
    this.chance = chance;
  ...
  public static CombinedIngredient empty() {
    CombinedIngredient r = new CombinedIngredient(Kind.ITEM, Ten4.vanillaId("air"), 0, 1);
    r.isEmpty = true;
    return r;
  }

  public static CombinedIngredient of(Kind kind, ResourceLocation id, int count, double chance) {
    CombinedIngredient r = new CombinedIngredient(kind, id, count, chance);
    r.resolve();
    return r;
  }

  public static CombinedIngredient fromJson(com.google.gson.JsonObject json) {
    if (json.has("item")) {
  ...
  public static CombinedIngredient fromNetwork(RegistryFriendlyByteBuf buf) {
