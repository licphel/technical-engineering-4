...
public class MacNBTManager
{
    ...
    public void rdt(CompoundTag nbt, HolderLookup.Provider registries)
    {

        for(int i = 0; i < t.inventory.getContainerSize(); i++) {
            t.inventory.setItem(i, ItemStack.parse(registries, nbt.getCompound("item" + i)).orElse(ItemStack.EMPTY));
        }
        for(int i = 0; i < t.upgradeSlots.inventory.getContainerSize(); i++) {
            t.upgradeSlots.inventory.setItem(i, ItemStack.parse(registries, nbt.getCompound("upgrade" + i)).orElse(ItemStack.EMPTY));
        }
        for(int i = 0; i < t.tanks.size(); i++) {
            t.tanks.get(i).readFromNBT(registries, nbt.getCompound("tank" + i));
        }
    ...
    public void wdt(CompoundTag nbt, HolderLookup.Provider registries)
    {

        for(int i = 0; i < t.inventory.getContainerSize(); i++) {
            nbt.put("item" + i, t.inventory.getItem(i).copy().save(registries));
        }
        for(int i = 0; i < t.upgradeSlots.inventory.getContainerSize(); i++) {
            nbt.put("upgrade" + i, t.upgradeSlots.inventory.getItem(i).copy().save(registries));
        }
        for(int i = 0; i < t.tanks.size(); i++) {
            CompoundTag tag = new CompoundTag();
            t.tanks.get(i).writeToNBT(registries, tag);
            nbt.put("tank" + i, tag);
        }
